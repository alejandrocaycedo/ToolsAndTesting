...
if ConectarDB == 0:
    ...
    prodVolumen = pd.read_csv(
        "C:/toolsandtesting/DataBase/PRODUCCION_Volumen.csv", sep=";"
    )
...
mapa = folium.Map(location=[7, -72.8], zoom_start=5)
# Pozos al mapa
for nomb, lat, lon in zip(namep, lati, longi):
    mc_pozos.add_child(
        folium.Marker(
            location=[float(lat), float(lon)],
            popup="<b> Pozo: </b> " + str(nomb),
            max_width=14000,
            min_width=10000,
            icon=folium.Icon(
                "green", icon_color="blue", icon="tower-observation", prefix="fa"
            ),
        )
    )
